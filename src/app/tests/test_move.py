...
class TestMoveToRoot:
    def test_move_sets_parent_and_previously_in(self, authed_client, db):
    ...
    def test_move_to_root_fails(self, authed_client, db):
        ...
        response = authed_client.post(
            reverse('app:item_action', kwargs={'pk': item.pk, 'action': 'move'}), {'barcode': ''}
        )
        ...
        assertMessages(
            response,
            [messages.Message(messages.ERROR, "Creating top-level items can't be done with this form.")],
        )
    ...
    def test_move_to_nonexistent_barcode_fails(self, authed_client, db):
        ...
        assertMessages(
            response,
            [messages.Message(messages.ERROR, "Destination item with barcode 'T=99999' not found")],
        )
    ...
    def test_move_to_unknown_foreign_barcode_fails(self, authed_client, db):
        ...
        assertMessages(
            response,
            [messages.Message(messages.ERROR, "Destination item with barcode 'xyzzy' not found")],
        )
    ...
    def test_move_to_external_barcode_fails(self, authed_client, db):
        ...
        assertMessages(
            response,
            [messages.Message(messages.ERROR, "Destination item with barcode 'UPC1234567890' not found")],
        )
    ...
    def test_move_into_own_descendant_fails(self, authed_client, db):
        ...
        assertMessages(
            response,
            [messages.Message(messages.ERROR, f'Cannot move item into its own descendant: {child.path}')],
        )
